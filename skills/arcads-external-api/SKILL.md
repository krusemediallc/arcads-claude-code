...
8. Compose JSON per OpenAPI / [reference.md](reference.md). **Primary video endpoint:** `POST /v2/videos/generate` with the appropriate `model` value (see `CreateVideoDto` in reference.md). Include `projectId` when the DTO supports it. Set `duration` based on script length for models that require it. For Nano Banana images, use `POST /v2/images/generate` with `model` set per the Nano Banana section (`nano-banana-2` unless the user chose Pro).
   ...
   - **⚠️ Seedance 2.0 v2v + human faces in reference video (confirmed 2026-04-09):** On v2v calls, reference videos containing people/faces consistently trip the content checker and get charged anyway (credits burn before content check). Use **product-only / object / abstract** reference videos. A clean bootstrap: generate a product shot via i2v or text-only first, then use that as the v2v reference. See `reference.md`.
   - **⚠️ Seedance 2.0 audio+image 500 regression (observed 2026-04-09):** `audioEnabled: true` combined with `referenceImages` may return `HTTP 500 UNKNOWN_ERROR` — server-side regression. Before each session, run a minimal sanity probe. If still broken, drop either audio or refImages until resolved. See `reference.md`.
